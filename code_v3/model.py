"""
WGQModel — Word-Gaze-Question model (V3).

V3 changes vs V2:
  - Mean+max pooling in Stream 2 (proposed in the original proposal but not yet implemented).
    Each passage word is pooled via both mean and max over the attended representations,
    giving the classifier two complementary views:
      mean_pool: average attended signal across the passage
      max_pool:  peak attended signal (words the model is MOST confident are relevant)
    Concatenated: pooled = (B, 2*d) instead of (B, d).
    Classifier head updated: TEXT_DIM + 2*d + num_gaze_stats.

  - UNFREEZE_TOP_LAYERS 2 → 4 in config.py (no change needed here — handled by _setup_roberta_freezing).

All other architecture details identical to V2.
"""
...
def _setup_roberta_freezing(roberta: RobertaModel,
    ...
    """Freeze all RoBERTa params, then selectively unfreeze the top N encoder layers."""
...
class WGQModel(nn.Module):
    def __init__(
        ...
        TEXT_DIM = self.roberta.config.hidden_size  # 1024 for large
        ...
        # ── Classifier: TEXT_DIM + 2*d (mean+max pool) + num_gaze_stats ──────
        # V3: pooled dim is 2*d because we concatenate mean and max pool
    ...
    def _mean_max_pool(
        self,
        attended:  torch.Tensor,  # (B, W, d)
        word_mask: torch.Tensor,  # (B, W) — True=valid
    ) -> torch.Tensor:            # (B, 2*d)
        """
        Mean+max pooling over passage words.
        Mean captures the average attended signal.
        Max captures the peak (words the model is most confident are question-relevant).
        """
        wm        = word_mask.unsqueeze(-1).float()                       # (B, W, 1)
        mean_pool = (attended * wm).sum(1) / (wm.sum(1) + 1e-8)          # (B, d)

        # Mask padding positions with large negative before max
        attended_masked = attended - (1.0 - wm) * 1e9                    # (B, W, d)
        max_pool        = attended_masked.max(dim=1).values               # (B, d)
        max_pool        = max_pool.nan_to_num(0.0)
    ...
    def forward(
        ...
        # V3: mean+max pooling (was mean-only in V1/V2)
        pooled   = self._mean_max_pool(attended, word_mask)               # (B, 2*d)

        combined = torch.cat([text_cls, pooled, global_stats], dim=-1)   # (B, 1024+512+6)
...
class NoQCondModel(nn.Module):
    """A1: No question conditioning — self-attention over fused passage words."""
    def __init__(self, num_ia_features=config.NUM_IA_FEATURES, num_gaze_stats=config.NUM_GAZE_STATS,
        ...
        # V3: also use mean+max pooling for fair comparison
        self.classifier  = nn.Sequential(
